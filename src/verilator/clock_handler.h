...
typedef struct {
    ...
    bool zero_tick; // whether or not the clock's first tick will occur at 0ps
} verilator_clock;
...
void handle_clocks(verilator_clock* clocks, size_t num_clocks, uint64_t max_time_ps, clock_dump_function dump_function) {
    ...
    while (continue_ticking && time_ps <= max_time_ps) {
        ...
        for (int i = 0; i < num_clocks; i++) {
            ...
            // if the time is 0 ps and the clock is not supposed to tick at 0 ps, skip it
            if (time_ps == 0 && !clock->zero_tick) {
                continue;
            }
